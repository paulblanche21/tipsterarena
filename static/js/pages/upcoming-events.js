...
async function fetchEventsForSport(sport) {
    ...
    if (sport === 'football') {
        try {
            // Fetch fixtures
            let response = await fetch('/api/football/fixtures/?category=fixtures', {
            ...
            // Fetch results
            response = await fetch('/api/football/fixtures/?category=results', {
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include',
            });
            if (!response.ok) throw new Error(`HTTP error: ${response.status} for football results`);
            data = await response.json();
            console.log(`Fetched ${data.length} football results from Django API`);
            allEvents = allEvents.concat(data);

            // Fetch inplay
            response = await fetch('/api/football/fixtures/?category=inplay', {
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include',
            });
            if (!response.ok) throw new Error(`HTTP error: ${response.status} for football inplay`);
            data = await response.json();
            console.log(`Fetched ${data.length} football inplay from Django API`);
            allEvents = allEvents.concat(data);
        } catch (error) {
            console.error(`Error fetching football fixtures/results/inplay:`, error);
        }
    } else {
        ...
        const handler = async (e) => {
            ...
            if (!isVisible) {
                ...
                if (card.classList.contains('carousel-event-card') && card.dataset.eventId) {
                    await window.fetchMatchDetails(card.dataset.eventId, details);
                }
            } else {
