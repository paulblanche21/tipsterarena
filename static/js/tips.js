...
function setupTipInteractions() {
  console.log('Initializing tip interactions');
...
function setupReplyModal() {
  console.log('Initializing reply modal');
  const commentModal = document.getElementById('comment-modal');
  if (!commentModal) {
  ...
  if (!replyInput) {
    console.warn('Comment input not found in setupReplyModal');
    return;
  }
...
function openCommentModal(tip, tipId, parentId = null) {
  ...
  if (!commentModal || !modalTip || !modalTipAvatar || !modalTipContent || !commentList || !commentSubmit) {
  ...
  const usernameElement = tipContent.querySelector('.tip-username');
...
function attachCommentActionListeners() {
  const commentModal = document.getElementById('comment-modal');
  if (!commentModal) {
    console.warn('Comment modal not found in attachCommentActionListeners');
    return;
  }
  const commentList = commentModal.querySelector('.comment-list');
